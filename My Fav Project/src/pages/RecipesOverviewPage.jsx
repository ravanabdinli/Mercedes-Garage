...
const RecipesOverviewPage = () => {
  ...
  const handleReorder = (sourceIndex, destinationIndex, options = {}) => {
    ...
    // 4) Reassign 'order' for each item
    updated.forEach((r, idx) => {
      r.order = idx + 1;
    });

    // 5) Update local state so UI refreshes
    setRecipes(updated);

    // 6) If 'inMemoryOnly' => skip the server patch
    if (options.inMemoryOnly) {
      return;
    }

    // 7) If 'patchServer' => do the PATCH calls now
  ...
  const handleShare = () => {
    // Double-check for selected recipes
    if (selectedRecipes.length === 0) {
      alert("No recipes selected!");
      return;
    }
    ...
    emailjs
      .send(
        "service_uy3kqbb",   // Replace with your EmailJS service ID
        "template_k053bmp",  // Replace with your EmailJS template ID
        emailParams,
        "k-B7CRJkdywHyeo-5"  // Replace with your EmailJS Public Key
      )
      .then(() => {
